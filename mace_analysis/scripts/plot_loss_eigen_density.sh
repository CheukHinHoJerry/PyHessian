...
for SEED in "${SEED_LIST[@]}"; do
    for SHUFFLE_SEED in "${SHUFFLE_SEED_LIST[@]}"; do
        ...
        OUTPUT_DIR="/scratch/st-ortner-1/jerry528/ETN/PyHessian/mace_analysis/results/water/sym_cp_${SEED}"
        DEVICE="cuda"
        BATCH_SIZE=10
        NDATA=-1

        # Run the Python script
        python /scratch/st-ortner-1/jerry528/ETN/PyHessian_2/mace_analysis/mace_pyhessian_loss_hessian_density.py --model_path "$MODEL_PATH" --config_path "$CONFIG_PATH" --output_dir "$OUTPUT_DIR" --device "$DEVICE" --batch_size "$BATCH_SIZE" --shuffle_seed "$SHUFFLE_SEED" --Ndata "$NDATA"
    done
done